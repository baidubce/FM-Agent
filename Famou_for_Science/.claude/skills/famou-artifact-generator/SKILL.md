...
- 如果有数据，首先分析和理解数据，例如数据等格式、可能的问题描述等。
...
基于已确认的 `problem.md`，生成并验证 FaMou 进化任务所需要的 `evaluator.py`、`init.py` 和 `prompt.md`。
